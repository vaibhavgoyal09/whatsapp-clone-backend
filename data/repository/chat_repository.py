...
class ChatRepository:
    ...
    async def create_new_group_chat(self, group_id: str, user_ids: List[str]) -> str:
        chat = {
            "user_ids": user_ids,
            "group_id": group_id,
    ...
    async def add_participants(self, group_id: str, user_ids: List[str]):
        await self.chat_collection.update_one(
            {"group_id": group_id}, {"$push": {"user_ids": {"$each": user_ids}}}
        )

    async def remove_participants(self, group_id: str, user_ids: List[str]):
        await self.chat_collection.update_one(
            {"group_id": group_id}, {"$pullAll": {"user_ids": user_ids}}
        )
